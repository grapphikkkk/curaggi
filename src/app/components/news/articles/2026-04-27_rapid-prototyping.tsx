import type { Article } from "./types";

// ─────────────────────────────────────────────
//  Article: 机上の空論に早くおさらば。
//           弊社が提供する爆速プロトタイピング支援
// ─────────────────────────────────────────────

const PROCESS_STEPS = [
  "企画・構想 / 市場調査",
  "ターゲット明確化",
  "コンセプト",
  "画面の試作（プロトタイピング）",
  "ユーザー検証・改善",
  "画面デザイン",
  "開発要件定義",
  "開発・運用",
];

const COLORS = {
  purple: "#9D35E4",
  teal: "#00CEC5",
  yellow: "#F2E72B",
  ink: "#0A0A0B",
  inkSoft: "#44444F",
  border: "#E2E2E8",
  bg: "#F8F8FB",
};

function ProcessFlow() {
  return (
    <div
      style={{
        margin: "var(--space-8) 0",
        padding: "var(--space-6)",
        background: COLORS.bg,
        borderRadius: "12px",
        border: `1px solid ${COLORS.border}`,
      }}
    >
      <div
        style={{
          fontFamily: "var(--font-display)",
          fontSize: "var(--text-xs)",
          fontWeight: 700,
          letterSpacing: "0.08em",
          textTransform: "uppercase",
          color: COLORS.inkSoft,
          marginBottom: "var(--space-4)",
        }}
      >
        Process — プロダクトが形になるまで
      </div>
      <ol
        style={{
          display: "flex",
          flexWrap: "wrap",
          gap: "8px",
          padding: 0,
          margin: 0,
          listStyle: "none",
        }}
      >
        {PROCESS_STEPS.map((label, i) => (
          <li
            key={i}
            style={{
              display: "flex",
              alignItems: "center",
              gap: "8px",
              flex: "1 1 160px",
              minWidth: "160px",
            }}
          >
            <span
              style={{
                display: "inline-flex",
                alignItems: "center",
                justifyContent: "center",
                width: "26px",
                height: "26px",
                borderRadius: "50%",
                background: COLORS.ink,
                color: "#ffffff",
                fontFamily: "var(--font-display)",
                fontWeight: 700,
                fontSize: "13px",
                flexShrink: 0,
              }}
            >
              {i + 1}
            </span>
            <span
              style={{
                fontFamily: "var(--font-body), var(--font-jp)",
                fontSize: "13px",
                fontWeight: 600,
                color: COLORS.ink,
                lineHeight: 1.4,
              }}
            >
              {label}
            </span>
          </li>
        ))}
      </ol>
    </div>
  );
}

function TimelineCompare() {
  // Compares industry standard vs Curaggi's pace.
  return (
    <div
      style={{
        margin: "var(--space-8) 0",
        padding: "var(--space-6)",
        background: COLORS.bg,
        borderRadius: "12px",
        border: `1px solid ${COLORS.border}`,
      }}
    >
      <div
        style={{
          fontFamily: "var(--font-display)",
          fontSize: "var(--text-xs)",
          fontWeight: 700,
          letterSpacing: "0.08em",
          textTransform: "uppercase",
          color: COLORS.inkSoft,
          marginBottom: "var(--space-5)",
        }}
      >
        Speed — 構想からプロトタイプまでの期間
      </div>

      {/* Industry standard */}
      <div style={{ marginBottom: "var(--space-5)" }}>
        <div
          style={{
            fontFamily: "var(--font-display), var(--font-jp)",
            fontSize: "13px",
            fontWeight: 700,
            color: COLORS.inkSoft,
            marginBottom: "6px",
          }}
        >
          一般的な進め方
        </div>
        <div
          style={{
            display: "flex",
            alignItems: "center",
            gap: "10px",
          }}
        >
          <div
            style={{
              flex: 1,
              height: "20px",
              background: "#C4C4CD",
              borderRadius: "10px",
            }}
          />
          <span
            style={{
              fontFamily: "var(--font-display)",
              fontSize: "14px",
              fontWeight: 700,
              color: COLORS.ink,
              whiteSpace: "nowrap",
            }}
          >
            2〜3ヶ月
          </span>
        </div>
      </div>

      {/* Curaggi */}
      <div>
        <div
          style={{
            fontFamily: "var(--font-display), var(--font-jp)",
            fontSize: "13px",
            fontWeight: 700,
            color: COLORS.purple,
            marginBottom: "6px",
          }}
        >
          Curaggi
        </div>
        <div
          style={{
            display: "flex",
            alignItems: "center",
            gap: "10px",
          }}
        >
          <div
            style={{
              width: "22%",
              height: "20px",
              background: COLORS.purple,
              borderRadius: "10px",
            }}
          />
          <span
            style={{
              fontFamily: "var(--font-display)",
              fontSize: "14px",
              fontWeight: 700,
              color: COLORS.purple,
              whiteSpace: "nowrap",
            }}
          >
            最短2週間
          </span>
        </div>
      </div>
    </div>
  );
}

function DomainMap() {
  // Three columns: 戦略・要件定義 / UIデザイン / エンジニア
  const cols = [
    {
      title: "戦略・要件定義領域",
      sub: "UX（ユーザー体験）デザイン",
      desc: "見える部分の背景となる、企画構想〜要件定義の領域",
      color: COLORS.purple,
      isOurs: true,
    },
    {
      title: "UIデザイン領域",
      sub: "Visual / Interaction",
      desc: "画面の色、画像、ボタン配置など、見える部分をつくる領域",
      color: COLORS.teal,
      isOurs: false,
    },
    {
      title: "エンジニア領域",
      sub: "Implementation",
      desc: "設計に基づいて実装・運用する領域",
      color: COLORS.yellow,
      isOurs: false,
    },
  ];
  return (
    <div
      style={{
        margin: "var(--space-8) 0",
        display: "grid",
        gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))",
        gap: "12px",
      }}
    >
      {cols.map((c, i) => (
        <div
          key={i}
          style={{
            position: "relative",
            padding: "var(--space-5)",
            border: `2px solid ${c.isOurs ? c.color : COLORS.border}`,
            borderRadius: "12px",
            background: c.isOurs ? `${c.color}10` : "#ffffff",
          }}
        >
          {c.isOurs && (
            <span
              style={{
                position: "absolute",
                top: -10,
                left: 12,
                background: c.color,
                color: "#ffffff",
                fontFamily: "var(--font-display)",
                fontSize: "11px",
                fontWeight: 700,
                letterSpacing: "0.08em",
                padding: "2px 8px",
                borderRadius: "6px",
              }}
            >
              OUR FOCUS
            </span>
          )}
          <div
            style={{
              fontFamily: "var(--font-display)",
              fontSize: "11px",
              fontWeight: 700,
              letterSpacing: "0.08em",
              textTransform: "uppercase",
              color: c.color,
              marginBottom: "6px",
            }}
          >
            {c.sub}
          </div>
          <div
            style={{
              fontFamily: "var(--font-display), var(--font-jp)",
              fontSize: "16px",
              fontWeight: 700,
              color: COLORS.ink,
              marginBottom: "8px",
            }}
          >
            {c.title}
          </div>
          <div
            style={{
              fontFamily: "var(--font-body), var(--font-jp)",
              fontSize: "13px",
              color: COLORS.inkSoft,
              lineHeight: 1.7,
            }}
          >
            {c.desc}
          </div>
        </div>
      ))}
    </div>
  );
}

function StrengthCard({
  num,
  title,
  body,
  color,
}: {
  num: string;
  title: string;
  body: string;
  color: string;
}) {
  return (
    <div
      style={{
        padding: "var(--space-6)",
        border: `2px solid ${color}`,
        borderRadius: "12px",
        background: `${color}10`,
        marginBottom: "var(--space-4)",
      }}
    >
      <div
        style={{
          display: "flex",
          alignItems: "baseline",
          gap: "10px",
          marginBottom: "8px",
        }}
      >
        <span
          style={{
            fontFamily: "var(--font-display)",
            fontSize: "28px",
            fontWeight: 800,
            color,
            lineHeight: 1,
          }}
        >
          {num}
        </span>
        <span
          style={{
            fontFamily: "var(--font-display), var(--font-jp)",
            fontSize: "18px",
            fontWeight: 700,
            color: COLORS.ink,
            lineHeight: 1.4,
          }}
        >
          {title}
        </span>
      </div>
      <div
        style={{
          fontFamily: "var(--font-body), var(--font-jp)",
          fontSize: "14px",
          color: COLORS.inkSoft,
          lineHeight: 1.85,
        }}
      >
        {body}
      </div>
    </div>
  );
}

function TableOfContents() {
  const items = [
    { href: "#section-1", label: "1. 議論が進まない場面ありませんか？" },
    { href: "#section-2", label: "2. 弊社の強み：意思決定・改善を高速化" },
    { href: "#section-3", label: "3. 事業領域ご紹介" },
  ];
  return (
    <nav
      aria-label="目次"
      style={{
        margin: "var(--space-8) 0",
        padding: "var(--space-6)",
        background: COLORS.bg,
        border: `1px solid ${COLORS.border}`,
        borderRadius: "12px",
      }}
    >
      <div
        style={{
          fontFamily: "var(--font-display)",
          fontSize: "11px",
          fontWeight: 700,
          letterSpacing: "0.1em",
          textTransform: "uppercase",
          color: COLORS.inkSoft,
          marginBottom: "var(--space-3)",
        }}
      >
        目次 / Contents
      </div>
      <ol
        style={{
          listStyle: "none",
          padding: 0,
          margin: 0,
          display: "flex",
          flexDirection: "column",
          gap: "8px",
        }}
      >
        {items.map((it) => (
          <li key={it.href} style={{ margin: 0 }}>
            <a
              href={it.href}
              style={{
                fontFamily: "var(--font-display), var(--font-jp)",
                fontSize: "15px",
                fontWeight: 600,
                color: COLORS.ink,
                textDecoration: "none",
                borderBottom: `1px solid transparent`,
              }}
            >
              {it.label}
            </a>
          </li>
        ))}
      </ol>
    </nav>
  );
}

export const article: Article = {
  title:
    "机上の空論に早くおさらば。 - 弊社が提供する爆速プロトタイピング支援",
  date: "2026-04-27",
  category: "Insight",
  tags: ["プロトタイピング", "UXデザイン", "AI活用", "事業領域"],
  summary:
    "資料ベースの議論では合意形成が進まない…。動くプロトタイプを最短2週間で形にし、意思決定と改善を高速化する Curaggi の支援アプローチをご紹介します。",
  slug: "rapid-prototyping",

  content: () => (
    <>
      <p>
        パワーポイントの資料だけで議論を進めても、新規事業や新機能の合意形成はなかなか進みません。
        意思決定の手前で時間を溶かしてしまったり、実装後に「思っていたものと違う」という手戻りが発生してしまうことも少なくないのではないでしょうか。
      </p>
      <p>
        本記事では、Curaggi が提供する
        <strong>爆速プロトタイピング支援</strong>
        について、3つのトピックでご紹介します。
      </p>

      <TableOfContents />

      {/* ────────────────────────────── */}
      <h2 id="section-1">1. 議論が進まない場面ありませんか？</h2>

      <p>
        新規事業や新機能の検討を進める中で、こんな場面に心当たりはありませんか？
      </p>

      <ul>
        <li>
          新規事業・新機能のアイデアが、資料ベースの議論では経営層や関連部署の合意を得にくい
        </li>
        <li>
          ユーザーテストやヒアリングを、机上の仮説ではなく実際に動くプロトタイプで実施したい
        </li>
        <li>
          ベンダー選定時の RFP/RFI 作成段階で、発注側の要件イメージが固まりきらず、提案の質にばらつきが出てしまう
        </li>
        <li>
          実装後に「思っていたものと違う」という手戻りが発生し、開発コストが膨らんでしまう
        </li>
      </ul>

      <h3>なぜ、こうした事態が起きるのか</h3>

      <p>
        プロダクトが形になるまでには、企画・構想から開発・運用まで多くのフェーズがあります。一般的な進め方では、それぞれの領域を別々の担当者・会社が担うことが多く、初期フェーズで十分な共通認識が築けないまま下流工程に進んでしまうケースが頻発します。
      </p>

      <ProcessFlow />

      <p>そこで起きる代表的な「詰まり」は、次の3つです。</p>

      <ol>
        <li>
          <strong>初期プロセスの机上空論が長引く</strong>
          ：構想段階で「動かないモノ」しかなく、ステークホルダーの認識合わせに時間がかかる。期間とコストを掛けた割にアウトプットが低品質になりやすい。
        </li>
        <li>
          <strong>領域別に担当が分断され、橋渡し人材が不足</strong>
          ：クライアント / 各領域担当の間で言語が異なり、要件の解像度が落ちたまま下流に流れていく。後続フェーズで炎上する。
        </li>
        <li>
          <strong>設計力なき AI 活用</strong>
          ：AI を使ってもインプットの設計が雑だと、生成物は使い物にならない。AI で「速く作れる」ことと、「価値あるものを作れる」ことは別物。
        </li>
      </ol>

      <blockquote>
        共通する原因は、ひとつ。<br />
        構想段階に「動くもの」がないこと。
      </blockquote>

      {/* ────────────────────────────── */}
      <h2 id="section-2">2. 弊社の強み：意思決定・改善を高速化</h2>

      <p>
        Curaggi では、UX デザインの現場経験と AI 活用ノウハウを掛け合わせ、
        <strong>構想からプロトタイプまでを最短2週間</strong>
        で実現します。早期に「触れる・動く」プロトタイプがあることで、ステークホルダーの合意形成、ユーザー検証、ベンダー選定までのスピードと精度が段違いに変わります。
      </p>

      <TimelineCompare />

      <h3>3つの強み</h3>

      <StrengthCard
        num="01"
        title="AI 活用で、高速かつ高品質"
        body="単に AI で「安く・早く」作るのではなく、UX デザイナーとしての設計思考に裏打ちされた AI 活用により、早期から高品質なアウトプットを形にします。"
        color={COLORS.purple}
      />
      <StrengthCard
        num="02"
        title="企画構想〜開発要件定義まで一貫支援"
        body="リサーチ・要件定義から、プロトタイピング、UI 設計、改善指示書まで一気通貫でご支援します。フェーズや領域をまたぐ「翻訳工数」を削減し、認識のズレを未然に防ぎます。"
        color={COLORS.teal}
      />
      <StrengthCard
        num="03"
        title="他領域との橋渡し"
        body="大手総合コンサルティングファーム・大手デジタルマーケティング代理店で現場を知り尽くした経験を活かし、コンサル・エンジニア・事業部門など他領域との橋渡しを担います。要件と実装、その間にある「認識のグレーゾーン」を埋める役割を果たします。"
        color={COLORS.yellow}
      />

      {/* ────────────────────────────── */}
      <h2 id="section-3">3. 事業領域ご紹介</h2>

      <p>
        Curaggi が支援するのは、プロダクト（アプリ / ウェブサイト / 社内システムなど）が形になるまでのプロセスのうち、
        <strong>UX（ユーザー体験）デザイン領域</strong>
        を中心とした上流〜中流工程です。
      </p>

      <DomainMap />

      <h3>具体的な支援メニュー</h3>

      <ul>
        <li>企画・構想 / 市場調査・ユーザーリサーチ</li>
        <li>ターゲット明確化 / ペルソナ・カスタマージャーニー設計</li>
        <li>コンセプト設計 / サービスブループリント</li>
        <li>画面の試作（プロトタイピング） / 動くモックの作成</li>
        <li>ユーザー検証・改善 / PoC・ユーザーテスト</li>
        <li>UI 設計 / デザインシステム構築 / CMS 設計</li>
        <li>開発要件定義 / RFP・RFI 作成支援</li>
        <li>AI / デザイン活用に関するセミナー・社内研修</li>
      </ul>

      <h3>こんな企業様におすすめ</h3>

      <ul>
        <li>
          新規事業・新機能の構想を、早期にステークホルダーと擦り合わせたい事業会社
        </li>
        <li>
          RFP/RFI 段階で要件イメージを固め、ベンダーへの伝達精度を上げたい発注企業
        </li>
        <li>AI を活用しつつも、安易な丸投げではなく価値ある成果物を作りたい組織</li>
        <li>UX デザイン / AI 活用について、社内のリテラシー向上を図りたい企業・スクール</li>
      </ul>

      <hr />

      <h2>おわりに</h2>

      <p>
        「机上の空論」を、できるだけ早く「動くプロトタイプ」に変える。
        それだけで、プロジェクト全体の質とスピードは大きく変わります。
      </p>
      <p>
        プロジェクトの初期構想段階からのご相談、AI を活用した社内研修のご依頼など、
        お気軽にお問い合わせください。
      </p>

      <p style={{ textAlign: "center", marginTop: "var(--space-10)" }}>
        <a
          href="/contact"
          style={{
            display: "inline-block",
            padding: "14px 28px",
            background: COLORS.ink,
            color: "#ffffff",
            fontFamily: "var(--font-display), var(--font-jp)",
            fontSize: "15px",
            fontWeight: 700,
            textDecoration: "none",
            borderRadius: "999px",
            letterSpacing: "0.02em",
          }}
        >
          お問い合わせ →
        </a>
      </p>
    </>
  ),
};
